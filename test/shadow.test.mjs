...
describe("Shadow", () => {
  ...
  it("should render correctly with skinning using pixi *.*.*", async () => {
    let render = (renderer, resources) => {
      let container = new PIXI3D.Container3D()
      container.y = -0.8
      ...
      light.rotationQuaternion.setEulerAngles(35, 210, 0)
      lightingEnvironment.lights.push(light)

      let model = container.addChild(PIXI3D.Model.from(resources["assets/skinned/skinned.gltf"].gltf))
      model.meshes.forEach(mesh => {
        mesh.material.lightingEnvironment = lightingEnvironment
      })
    ...
    await expect(render).to.match("snapshots/cesxw.png", {
      resources: [
